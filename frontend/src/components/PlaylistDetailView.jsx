// frontend/src/components/PlaylistDetailView.jsx
import React, { useState, useEffect, useRef } from 'react';
import { getPlaylists, removeVideoFromPlaylist } from '../api/playlistStorage.js';
...
// Lazy loading video thumbnail
function LazyVideoThumbnail({ video, index, isActive, onClick, onRemove }) {
...
export default function PlaylistDetailView({ playlistId }) {
  ...
  useEffect(() => {
    ...
    setPlaylist(found);
    if (found && found.videos.length > 0 && !selectedVideo) {
      const firstVideo = found.videos[0];
  ...
  const handleRemoveVideo = (videoUrl, e) => {
    ...
    setPlaylist(updated);
    setToastMessage('Video playlist\'ten çıkarıldı');

    if (selectedVideo?.['optimized-url'] === videoUrl) {
      if (updated && updated.videos.length > 0) {
        handleVideoSelect(updated.videos[0]);
      } else {
  ...
  const handleNextVideo = () => {
    ...
    const currentIndex = playlist.videos.findIndex(v => v['optimized-url'] === selectedVideo['optimized-url']);
    if (currentIndex < playlist.videos.length - 1) {
      handleVideoSelect(playlist.videos[currentIndex + 1]);
    }
  };
  ...
  const getNextVideo = () => {
    if (!selectedVideo || !playlist) return null;
    const currentIndex = playlist.videos.findIndex(v => v['optimized-url'] === selectedVideo['optimized-url']);
    if (currentIndex < playlist.videos.length - 1) {
        ...
        {!cinemaMode && (
          <div className="lg:col-span-1">
            <div className="bg-white rounded shadow p-4">
              <h3 className="font-semibold text-gray-800 mb-4">{playlist.name}</h3>
